...
fn print_help() {
    ...
    println!("  wtf key issue <name>                       provision a device key");
    ...
    match sub {
        "issue" => {
            let Some(name) = args.get(1) else {
                eprintln!("usage: wtf key issue <name>");
                return 2;
            };
...
fn cmd_setup(args: &[String]) -> i32 {
    ...
    match client::get_text(&format!("{}/healthz", cfg.hub_url)) {
        ...
        Ok((status, _)) => {
            eprintln!("error: hub responded HTTP {status} (is `wtf serve` running there?)");
            return 1;
        }
        Err(e) => {
    ...
    if let Err(e) = mcp::fetch_state(&cfg) {
        eprintln!("error: config saved, but credential check against the hub failed: {e}");
        return 1;
    }
